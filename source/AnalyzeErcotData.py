...
def plot_with_excess_capacity(top_dir, load_data_df):
    ...
    for filename in glob.glob(f'{top_dir}/data/daily_ev_load_*.csv'):
        ...
        for month in range(1,13):
            ...
            aggregated_data_df['Mean Excess (Month) (MW)'] = aggregated_data_df['Max Load (MW)'] - aggregated_data_df['mean']
            aggregated_data_df['Mean Excess (Month) + std (MW)'] = aggregated_data_df['Max Load (MW)'] - aggregated_data_df['mean'] + aggregated_data_df['std']
            aggregated_data_df['Mean Excess (Month) - std (MW)'] = aggregated_data_df['Max Load (MW)'] - aggregated_data_df['mean'] - aggregated_data_df['std']
            aggregated_data_df['Max Excess (Month) (MW)'] = aggregated_data_df['Max Load (MW)'] - aggregated_data_df['min']
            aggregated_data_df['Min Excess (Month) (MW)'] = aggregated_data_df['Max Load (MW)'] - aggregated_data_df['max']
        
            # Calculate the mean (+/-std), max and min excess based on the maximum load over the year
            aggregated_data_df['Mean Excess (Year) (MW)'] = max_load - aggregated_data_df['mean']
            aggregated_data_df['Mean Excess (Year) + std (MW)'] = max_load - aggregated_data_df['mean'] + aggregated_data_df['std']
            aggregated_data_df['Mean Excess (Year) - std (MW)'] = max_load - aggregated_data_df['mean'] - aggregated_data_df['std']
            aggregated_data_df['Max Excess (Year) (MW)'] = max_load - aggregated_data_df['min']
            aggregated_data_df['Min Excess (Year) (MW)'] = max_load - aggregated_data_df['max']
            ...
            ax.axhline(max_load, label=f'Max Historical Load for Year', color='blue', linewidth=2, linestyle='--', zorder=100)
            
            handles, labels = ax.get_legend_handles_labels()
            
            mean_line, = ax.plot(aggregated_data_df['Mean Excess (Year) (MW)'], linewidth=3, color='navy')
            std_patch = ax.fill_between(aggregated_data_df['Hour'], aggregated_data_df['Mean Excess (Year) - std (MW)'], aggregated_data_df['Mean Excess (Year) + std (MW)'], color='blue', alpha=0.4)
            extrema_patch = ax.fill_between(aggregated_data_df['Hour'], aggregated_data_df['Min Excess (Year) (MW)'], aggregated_data_df['Max Excess (Year) (MW)'], color='blue', alpha=0.2)
            ...
            plt.savefig(f'{top_dir}/plots/daily_ev_load_with_excess_{zone}_{month_label}_yearMax.png')
            plt.close()
...
def plot_coast_load(top_dir, load_data_df):
    ...
    for month in range(1,13):
        ...
        mean_line, = ax.plot(aggregated_data_dicts['coast'][month]['Mean Excess (Month) (MW)'], linewidth=3, color='navy')
        std_patch = ax.fill_between(aggregated_data_dicts['coast'][month]['Hour'], aggregated_data_dicts['coast'][month]['Mean Excess (Month) - std (MW)'], aggregated_data_dicts['coast'][month]['Mean Excess (Month) + std (MW)'], color='blue', alpha=0.4)
        extrema_patch = ax.fill_between(aggregated_data_dicts['coast'][month]['Hour'], aggregated_data_dicts['coast'][month]['Min Excess (Month) (MW)'], aggregated_data_dicts['coast'][month]['Max Excess (Month) (MW)'], color='blue', alpha=0.2)
        ...
        for zone in zone_mapping:
            if zone == 'coast':
                continue
            mean_line_others, = ax.plot(aggregated_data_dicts[zone][month]['Mean Excess (Month) (MW)'], linewidth=2, linestyle='--', color='red')
        ...
        labels = labels + ['Mean Excess + Stdev (MW)', 'Min/Max Excess (MW)', 'Other Zones']
        
        ax.legend(handles, labels, fontsize=16, ncol=2)
        
        plt.savefig(f'{top_dir}/plots/daily_ev_load_with_excess_coast_{month_label}_monthMax.png')
        plt.close()
        ...
        mean_line, = ax.plot(aggregated_data_dicts['coast'][month]['Mean Excess (Year) (MW)'], linewidth=3, color='navy')
        std_patch = ax.fill_between(aggregated_data_dicts['coast'][month]['Hour'], aggregated_data_dicts['coast'][month]['Mean Excess (Year) - std (MW)'], aggregated_data_dicts['coast'][month]['Mean Excess (Year) + std (MW)'], color='blue', alpha=0.4)
        extrema_patch = ax.fill_between(aggregated_data_dicts['coast'][month]['Hour'], aggregated_data_dicts['coast'][month]['Min Excess (Year) (MW)'], aggregated_data_dicts['coast'][month]['Max Excess (Year) (MW)'], color='blue', alpha=0.2)
        ...
        for zone in zone_mapping:
            if zone == 'coast':
                continue
            mean_line_others, = ax.plot(aggregated_data_dicts[zone][month]['Mean Excess (Year) (MW)'], linewidth=2, linestyle='--', color='red')
        ...
        plt.savefig(f'{top_dir}/plots/daily_ev_load_with_excess_coast_{month_label}_yearMax.png')
        plt.close()
