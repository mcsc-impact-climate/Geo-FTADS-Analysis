...
def calculate_emission_intensity_using_mpg_times_payload(df_lca_1mpg, df_mpg_times_payload):
    '''
    ...
    sum_of_weights = {}
    for column in ['WTP', 'PTW', 'WTW']:
        df_emission_intensity[column].values[:] = 0.
    
    for column in ['WTP', 'PTW', 'WTW']:
        df_emission_intensity[column] = df_lca_1mpg[column] / df_mpg_times_payload[0]
        df_emission_intensity_unc[column] = df_emission_intensity[column] * df_mpg_times_payload[1] / df_mpg_times_payload[0]
...
def evaluateGreetWtwTruck_by_mpg_times_payload(df_mpg_times_payload, faf5_commodity='all'):
    '''
    ...
    # Read in distribution of fuel efficiency / mpg for the given FAF5 commodity
    if aggregated_commodity == 'all':
        commodity_save = 'all'
    else:
        commodity_save = InfoObjects.FAF5_VIUS_commodity_map[aggregated_commodity]['short name']
    
    # Evaluate overall emission intensity (g / ton-mile)
    df_lca_1mpg = readGreetWtwTruck(f'{top_dir}/data/GREET_LCA/truck_heavy_gvw_diesel_1mpg_wtw.csv')
    
    # Evaluate the weighted average of LCAs
    df_emission_intensity, df_emission_intensity_unc = calculate_emission_intensity_using_mpg_times_payload(df_lca_1mpg, df_mpg_times_payload[aggregated_commodity])
...
def plot_truck_emissions_per_commodity(normalize_by_payload = False, use_vius_mpg = False, plot_unc=False):
    '''
    ...
    co2_emission_intensity = {'commodity': [], 'WTP': [], 'PTW': [], 'WTW': [], 'WTP unc': [], 'PTW unc': [], 'WTW unc': []}
...
#            co2_emission_intensity['WTW'].append(float(df_lca['WTW'][df_lca['Item'] == 'CO2 (w/ C in VOC & CO)']))
    ...
    for commodity in commodities:
        df_lca, df_lca_unc = evaluateGreetWtwTruck_by_mpg_times_payload(df_mpg_times_payload, commodity)
        co2_emission_intensity['commodity'].append(commodity)
        
        co2_emission_intensity['WTP'].append(float(df_lca['WTP'][df_lca['Item'] == 'CO2 (w/ C in VOC & CO)']))
        co2_emission_intensity['WTP unc'].append(float(df_lca_unc['WTP'][df_lca_unc['Item'] == 'CO2 (w/ C in VOC & CO)']))
        co2_emission_intensity['PTW'].append(float(df_lca['PTW'][df_lca['Item'] == 'CO2 (w/ C in VOC & CO)']))
        co2_emission_intensity['PTW unc'].append(float(df_lca_unc['PTW'][df_lca_unc['Item'] == 'CO2 (w/ C in VOC & CO)']))
        co2_emission_intensity['WTW'].append(float(df_lca['WTW'][df_lca['Item'] == 'CO2 (w/ C in VOC & CO)']))
        co2_emission_intensity['WTW unc'].append(float(df_lca_unc['WTW'][df_lca_unc['Item'] == 'CO2 (w/ C in VOC & CO)']))
    ...
    if plot_unc:
        plt.barh(co2_emission_intensity['commodity'], co2_emission_intensity['WTW'], xerr=co2_emission_intensity['WTW unc'], ecolor='black', capsize=5, alpha=0, zorder=1000)
    xlim = plt.xlim()
    ...
    print(f'Saving figure to plots/co2_emissions_per_commodity_mpg_times_payload.png')
    plt.savefig(f'plots/co2_emissions_per_commodity_mpg_times_payload.png')
    plt.savefig(f'plots/co2_emissions_per_commodity_mpg_times_payload.pdf')
...
def fillLcaDf(df_dict, top_dir, commodity='all'):
    ...
    # Uncomment this if evaluating emission intensities using distribution of fuel efficiency / payload
    df_mpg_times_payload = pd.read_csv(f'{top_dir}/data/VIUS_Results/mpg_times_payload.csv')
    df_lca_truck, df_lca_truck_unc = evaluateGreetWtwTruck_by_mpg_times_payload(df_mpg_times_payload, faf5_commodity=commodity)
