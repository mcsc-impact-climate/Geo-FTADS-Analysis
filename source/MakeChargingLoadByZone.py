...
def get_ev_load_profile(top_dir, load_profile_path):
    ...
    spline = UnivariateSpline(load_profile_df['Hours'], load_profile_df['Power (kW)'], s=500, ext=3)
...
def get_daily_ev_demands(top_dir, ev_load_data_gpd, ev_daily_load_profile_df):
    ...
    for zone in zones:
        ...
        for center in ev_load_data_zone_df['Nearest Center']:
            daily_ev_load_dict[center] = float(ev_load_data_zone_df['Av P Dem'][ev_load_data_zone_df['Nearest Center'] == center].iloc[0]) * ev_daily_load_profile_df['Power']
...
def main():
    ...
    ev_daily_load_profile_df = get_ev_load_profile(top_dir, f'{top_dir}/data/Borlaug_et_al_most_extreme_HDEV_load_profile.csv')
